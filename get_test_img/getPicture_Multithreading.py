...
class GetImg():
    ...
    def getgender(self, genconfid, tablename):
        tmpdb = MySQLdb.connect(host=host, port=port, user=user, passwd=passwd, db=db, charset="utf8")
    ...
    def getrace(self, raconfid, tablename):
        num = 0
        tmpdb = MySQLdb.connect(host=host, port=port, user=user, passwd=passwd,
                                db=db, charset="utf8")
        tmpcursor = tmpdb.cursor()
        # sql = "SELECT (gender & 15) AS gender1, f.image_data,f.json FROM intellif_face." + tablename + " f limit 100000;"
...
if __name__ == "__main__":
    ...
    for tablename in tablelist:
        ...
        t2 = threading.Thread(target = GetImg.getglasses, args = (confid, tablename))
        t3 = threading.Thread(target = GetImg.getage, args = (tablename,))
        t4 = threading.Thread(target = GetImg.gethat, args = (confid, tablename))
        t5 = threading.Thread(target = GetImg.getrace, args = (confid, tablename))
